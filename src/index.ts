    ...
    statusAsync()
        ...
        .catch(() => {
            stopAsync(req.body.user)
                ...
                .catch((err) => {
                    if (err == 'failed due to run interval')
                        post("停止コマンドを拒否しました", "前回の処理の実行から" + process.env.WAIT_SECONDS_FROM_LAST_PROCESS + "秒経過していないため、コマンドを拒否しました。", 2)
                    else
    ...
    statusAsync()
        ...
        .catch(() => {
            restartAsync(req.body.user)
                ...
                .catch((err) => {
                    if (err == 'failed due to run interval')
                        post("再起動コマンドを拒否しました", "前回の処理の実行から" + process.env.WAIT_SECONDS_FROM_LAST_PROCESS + "秒経過していないため、コマンドを拒否しました。", 2)
                    else
